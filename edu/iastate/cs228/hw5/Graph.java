...
public class Graph {
	...
	public void addEdge(int fromIdx, int toIdx, int index, String word) throws IndexOutOfBoundsException, NullPointerException {
		GraphEdge gEdge = new GraphEdge(index, word, vertices[fromIdx], vertices[toIdx]);
		GraphEdge gOtherEdge = new GraphEdge(index, word, vertices[toIdx], vertices[fromIdx]);
		vertices[fromIdx].edges().add(gEdge);
		vertices[toIdx].edges().add(gOtherEdge);
	}
