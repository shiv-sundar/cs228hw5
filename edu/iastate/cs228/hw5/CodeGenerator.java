...
public class CodeGenerator {
	...
	private void table(PrintStream ps, int[][] data, String access, String varName) {
		...
		for (int x = 0; x < data.length - 1; x++) {
			...
			ps.print(data[x][data[x].length - 1]);
			ps.print("},");
		}
		
		ps.println("{");
		for (int y = 0; y < data[data.length - 1].length - 1; y++) {
			ps.print(data[data.length - 1][y]);
			ps.print(", ");
		}
	...
	private void testMain(PrintStream ps, String keyArrName) {
		...
		ps.println();
		ps.print(INDENT[indentLevel + 3]);
		ps.printf("System.err.printf(\"Error while hashing \\\"%%s\\\": " + "expected %%d but was %%d%%n\", %s[i], i, h);", keyArrName);
		ps.println();
		ps.print(INDENT[indentLevel + 2]);
		ps.println("}");
		ps.print(INDENT[indentLevel + 1]);
